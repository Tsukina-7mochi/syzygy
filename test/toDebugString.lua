...
describe("toDebugString", function()
    ...
    describe("table", function()
        ...
        test("array with hole", function()
            expect(toDebugString({ [1] = 1, [5] = 3 })):toBe("{ 1, (empty x 3), 3 }")
        end)

        test("array with large hole", function()
            expect(toDebugString({ [1] = 1, [100] = 3 })):toBe("{ 1, [100] = 3 }")
        end)
        ...
        test("array start with 2", function()
            expect(toDebugString({ [2] = 2, [3] = 3, [4] = 4 })):toBe("{ [2] = 2, 3, 4 }")
        end)

        test("array start with -1", function()
            expect(toDebugString({ [-1] = -1, [0] = 0, 1 })):toBe("{ [-1] = -1, 0, 1 }")
        end)

        test("array with decimal index", function()
            expect(toDebugString({ 1, 2, [1.5] = 1.5 })):toBe("{ 1, [1.5] = 1.5, [2] = 2 }")
        end)

        test("simple map", function()
            expect(toDebugString({ key1 = "value1", key2 = "value2" })):toBe('{ key1 = "value1", key2 = "value2" }')
        end)

        test("array and map mixed", function()
            expect(toDebugString({ 1, 2, 3, foo = "bar" })):toBe('{ 1, 2, 3, foo = "bar" }')
        end)
    end)
end)
